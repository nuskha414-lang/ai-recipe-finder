...
export default function Page() {
  const [diet, setDiet] = useState<"Vegetarian" | "Vegan" | "Egg" | "Any">(
    "Vegetarian"
  );
  ...
  const payload = useMemo(
    () => ({ diet, cuisine, protein, time, allergies, ingredients, servings }),
    [diet, cuisine, protein, time, allergies, ingredients, servings]
  );
    ...
    <main className="min-h-screen bg-gray-50 text-gray-800">
      <div className="max-w-3xl mx-auto p-6">
        ...
        <p className="mb-6 text-gray-600">
          Answer a few inputs, then generate a recipe or jump to an official
          YouTube video.
        </p>
        ...
        <div className="grid gap-4 bg-white rounded-2xl p-5 shadow">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <label className="grid gap-1">
              ...
              <input
                ...
                placeholder="Paneer, chana, tofu, etc."
              />
            </label>
            ...
            <label className="grid gap-1">
              ...
              <input
                ...
                value={servings}
                onChange={(e) =>
                  setServings(parseInt(e.target.value || "0", 10))
                }
              />
            </label>
          ...
          <div className="grid md:grid-cols-2 gap-3 items-end">
            <button
              ...
              className="rounded-2xl px-4 py-3 bg-black text-white shadow disabled:opacity-50"
            >
            ...
            <div className="grid gap-1">
              ...
              <button
                onClick={findYouTube}
                disabled={loadingYT}
                className="rounded-2xl px-4 py-3 bg-red-600 text-white shadow disabled:opacity-50"
              >
                {loadingYT ? "Searching…" : "Find Official YouTube"}
              </button>
            </div>
          </div>
          ...
          {recipe && (
            <div className="grid gap-3 border-t pt-4">
              ...
              <p className="text-sm text-gray-600">
                Servings: {recipe.servings} · Time: ~{recipe.time} min
              </p>
              {recipe.macros && (
                <p className="text-sm text-gray-600">
                  Approx macros/serving: {recipe.macros.calories} kcal · P{" "}
                  {recipe.macros.protein}g · C {recipe.macros.carbs}g · F{" "}
                  {recipe.macros.fat}g
                </p>
              )}
              <div>
              ...
              <div>
                ...
                <ol className="list-decimal ml-6 space-y-1">
                  {recipe.steps?.map((it: string, i: number) => (
                    <li key={i}>{it}</li>
                  ))}
                </ol>
              </div>
              ...
              {recipe.youtube_suggestions?.length ? (
                <div className="border rounded-xl p-3 bg-gray-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recipe.youtube_suggestions.map(
                      (q: string, i: number) => (
                        <a
                          key={i}
                          className="underline"
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {videos.map((v) => (
                  <a
                    ...
                    target="_blank"
                    className="rounded-2xl overflow-hidden border hover:shadow"
                  >
                    <img
                      src={v.thumbnail}
                      alt={v.title}
        ...
        <footer className="text-xs text-gray-500 mt-6">
          Built for demo purposes. Nutrition is approximate; verify against your
          needs.
        </footer>
      </div>
    </main>
